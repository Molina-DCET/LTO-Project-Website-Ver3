...
/* ===================================================================
        ...
        =================================================================== */
        var WINDOW_SECTION = 'Cashier';
        var WINDOW_ID = 'cashier';
        ...
        function saveActiveCalling(ticket, startTime, isRecall) {
            if (ticket) {
        ...
        function callAgain() {
            ...
            saveActiveCalling(currentCalling, nowTime, true);
            startTimer(nowTime);
        ...
        function putOnHold() {
            ...
            currentCalling.heldByWindow = 'WINDOW K/L';
        ...
        function mToHold() {
            ...
            var h = JSON.parse(localStorage.getItem(LS_HOLD) || '[]');
            t.status = 'onhold';
            t.heldByWindow = _winLabel();
            t.currentWindow = 'On Hold at ' + _winLabel();
            _addHistory(t, 'Put on hold at ' + _winLabel(), 'ON_HOLD');
            h.push(t);
            localStorage.setItem(LS_HOLD, JSON.stringify(h));
        ...
        function mToWait() {
            ...
            if (!t) { closeModal(); return; }
            var rawH = JSON.parse(localStorage.getItem(LS_HOLD) || '[]');
            rawH = rawH.filter(function (item) { return getTicketKey(item) !== getTicketKey(t); });
            localStorage.setItem(LS_HOLD, JSON.stringify(rawH));
            var q = JSON.parse(localStorage.getItem(LS_QUEUE) || '[]');
            t.status = 'pending';
            t.currentWindow = 'Queue for ' + WINDOW_SECTION;
            _addHistory(t, 'Restored from hold at ' + _winLabel() + ' to pending queue', 'RESTORED');
            q.push(t);
